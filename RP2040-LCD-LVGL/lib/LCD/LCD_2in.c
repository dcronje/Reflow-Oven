...
******************************************************************************/
static void LCD_2IN_SendCommand(uint8_t Reg)
{
...
parameter:
    Data : Write data
******************************************************************************/
static void LCD_2IN_SendData_8Bit(uint8_t Data)
{
...
parameter:
    Data : Write data
******************************************************************************/
static void LCD_2IN_SendData_16Bit(uint16_t Data)
{
    DEV_Digital_Write(LCD_DC_PIN, 1);
    DEV_Digital_Write(LCD_CS_PIN, 0);
...
********************************************************************************/
static void LCD_2IN_SetAttributes(uint8_t Scan_dir)
{
...
parameter:
********************************************************************************/
void LCD_2IN_Init(uint8_t Scan_dir)
{
    DEV_SET_PWM(90);
    //Hardware reset
    LCD_2IN_Reset();
...
********************************************************************************/
void LCD_2IN_SetWindows(uint16_t Xstart, uint16_t Ystart, uint16_t Xend, uint16_t Yend)
{
...
void LCD_2IN_Clear(uint16_t Color)
{
	...
	for(i = 0; i < LCD_2IN_WIDTH; i++){
		DEV_SPI_Write_nByte(LCD_SPI_PORT,p,LCD_2IN_HEIGHT*2);
	}
    DEV_Digital_Write(LCD_CS_PIN, 1);
}
...
void LCD_2IN_Display(uint8_t *Image)
{
    ...
    for (j = 0; j < LCD_2IN.WIDTH; j++) {
        DEV_SPI_Write_nByte(LCD_SPI_PORT,(uint8_t *)Image+LCD_2IN.HEIGHT*2*j,LCD_2IN.HEIGHT*2);
    }
    DEV_Digital_Write(LCD_CS_PIN, 1);
    LCD_2IN_SendCommand(0x29);
}

void LCD_2IN_DisplayWindows(uint16_t Xstart, uint16_t Ystart, uint16_t Xend, uint16_t Yend, uint16_t *Image)
{
    // display
    uint32_t Addr = 0;
...
void LCD_2IN_DisplayPoint(uint16_t X, uint16_t Y, uint16_t Color)
{
